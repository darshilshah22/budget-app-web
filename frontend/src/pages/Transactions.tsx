...
import EmptyTransactionsView from "../components/transactions/EmptyTransactionsView";
...
export default function Transactions() {
  ...
  const { transactions, isLoading, pagination } = useAppSelector(
  ...
  const handleEdit = async (transaction: Transaction) => {
    ...
    await dispatch(updateTransaction({ id: transaction._id, data: transaction })).unwrap();
  ...
  return (
    <>
                          ...
                          <td className="py-4 px-6">
                            <div className="flex items-center justify-center space-x-2">
                              <motion.button
                                ...
                                whileTap={{ scale: 0.9 }}
                                onClick={() => {
                                  setShowEditTransaction(transaction._id);
      ...
      <Modal
        isOpen={showAddTransaction || showEditTransaction !== null}
        onClose={() => {
          setShowAddTransaction(false);
          setShowEditTransaction(null);
        }}
        title={showAddTransaction ? "Add New Transaction" : "Edit Transaction"}
      >
        <AddTransactionForm
          onSubmit={showAddTransaction ? handleAddTransaction : handleEdit}
          onCancel={() => setShowAddTransaction(false)}
          transaction={showEditTransaction ? transactions.find((transaction: Transaction) => transaction._id === showEditTransaction) : undefined}
        />
        </Modal>
    </>
  );
